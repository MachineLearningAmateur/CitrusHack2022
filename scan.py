        ...
        for (top, right, bottom, left), name in zip(face_locations, face_names):
            ...
            if name == "Unknown":
                ...
                if name != prevName:
                    start = time.time()
                prevName = name
                cv2.putText(frame, ref_dict[name], (left + 6, bottom + 20), font, getFontScale(ref_dict[name], right - left), (255, 255, 255), 1)
                end = time.time() #end of timer
                print(end - start)
                if (end - start >= 2):
                    print(ref_dict[name])
                    classRoster[ref_dict[name]] = True
...
for key, name in ref_dict.items():
    classRoster[name] = False #False means absent; True means present
print(ref_dict)
showClassRoster()
